...
use pallet_dao_core::{Pallet as DaoCore, Config as DaoConfig, Currency};
...
fn add_voters<T: Config>(proposal_id: Vec<u8>, n: u32) {
	for i in 0..n {
		let voter = account("voter", i, SEED);//whitelisted_caller();
		assert!(
			Votes::<T>::vote(RawOrigin::Signed(voter).into(), proposal_id.clone(), Some(true)).is_ok()
		);
	}
}
...
fn setup_dao<T: Config>(caller: T::AccountId) -> Vec<u8> {
	...
	assert!(DaoCore::<T>::issue_token(origin.clone().into(), dao_id.clone(), 1000_u32.into()).is_ok());
	dao_id
}
...
fn setup_dao_with_governance<T: Config>(caller: T::AccountId) -> Vec<u8> {
	...
	let proposal_duration = 1_u32.into();
...
benchmarks! {
	create_proposal {
		...
		// https://en.wikipedia.org/wiki/SHA-3#Examples_of_SHA-3_variants
		let hash = b"a7ffc6f8bf1ed76651c14756a061d662f580ff4de43b49fa82d80a4b80f8434a".to_vec();
	}: _(RawOrigin::Signed(caller.clone()), dao_id.clone(), proposal_id.clone(), metadata, hash)
	verify {
	...
	fault_proposal {
		let caller = setup_caller::<T>();
		let dao_id = setup_dao_with_governance::<T>(caller.clone());
		let proposal_id = setup_proposal::<T>(caller.clone(), dao_id.clone());
